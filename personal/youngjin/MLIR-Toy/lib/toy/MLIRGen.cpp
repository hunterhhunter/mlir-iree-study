...
private:
  ...
  // 주어진 표현식(expr)이 결과값의 구조체 정의(StructAST)를 반환.
  // a.b가 입력되면 멤버 b의 구조체 타입을 찾아 반환함. = a의 VarDeclExprAST
  // 주로 구조체 멤버 접근 연산시 LHS의 타입을 알아낼 때 사용
  ...
  // 접근식(a.b)에서 b가 메모리상 몇 번째 오프셋인지 계산하는 함수.
  // var.real에서 real이 몇번째 멤버인지 확인하기 위해.
