...
private:
  ...
  std::unique_ptr<ExprAST> parseTensorLiteralExpr() {
    ...
    do {
      // number literal 또는 또다른 중첩 배열을 가질 수 있음
      if (lexer.getCurToken() == '[') {
        values.push_back(parseTensorLiteralExpr());
        // back()은 쓰면  nullptr인지 확인 가능 / empty()는 요소 개수만 반환
        // Q: empty 쓰지 않는 이유 (답변: push_back 직후라 empty는 항상 false임. 값의 유효성을 봐야 함)
        if (!values.back())
  ...
  /// binary Expression의 우변을 재귀적으로 구문분석
  ...
  /// 파싱 중 에러 신호를 돕는 함수
  /// 입력으로 기대되는 토큰과 context를 제공하는 다른 인자를 입력받음.
